...
class TestListingSource:
    def test_init(self) -> None:
        ...
        source = ListingSource(relative, output)
...
class TestListingNode:
    def test_init(self) -> None:
        node = ListingNode(True)
        assert node.leaf

    def test_children_empty(self) -> None:
        node = ListingNode(False)
...
class TestListingBuilder:
    def test_build_processes_listing_sources(
        ...
        source = ListingSource(PurePath("docs"), PurePath("docs/index"))
...
def test_render_html_produces_links() -> None:
    ...
    entry_source = MockSource(
        relative_path=PurePath("docs/module.py"),
        output_path=PurePath("docs/module"),
    )
    ...
    listing = Listing()
    listing.add_source(entry_source)
    listing.add_source(listing_source)
    listing.add_source(entry_source)
    node = ListingNode(False)

    context = Context({Source: listing_source, Listing: listing})
    parent = HTMLTag("div")
